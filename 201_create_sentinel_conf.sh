...
#FAILOVER_PL_OPTS="--sentinel localhost:$PORT" 
...
for i in `seq 1 $NUM`; do
	...
	if [ $IS_SLAVE ]; then
		echo "sentinel monitor redis_$i $MASTER $REDIS_PORT 1" >>$CONF_FILE
	else
		echo "sentinel monitor redis_$i $HOST $REDIS_PORT 1" >>$CONF_FILE
	fi
